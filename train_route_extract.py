...
def train_ae(obs_len, pred_len):
    ...
    dataloader = DataLoader(param_config.batches_path, param_config.data_scale, param_config.lon_min,
                            param_config.lon_max, param_config.lat_min, param_config.lat_max, param_config.random_state,
                            model_config.obs_len, model_config.pred_len)
    traj_train = []
    traj_len = model_config.obs_len + model_config.pred_len
    param_config.num_epochs = 1000
    ...
    torch.save(model, f'trained_model/route_{obs_len}_{pred_len}.pth')


def route_extract(dataloader, obs_len, pred_len, seed, device, n_clusters):
    traj_len = obs_len + pred_len
    if not os.path.exists(f'./trained_model/route_{obs_len}_{pred_len}.pth'):
        train_ae(obs_len, pred_len)
