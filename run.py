...
def main(output_dir='./output'):
    ...
    dataloader = DataLoader(param_config.batches_path, param_config.data_scale, param_config.lon_min, param_config.lon_max, param_config.lat_min, param_config.lat_max, param_config.random_state)
    ...
    print(len(dataloader.traj_train), len(dataloader.traj_test))
    exit()
    kmeans, ae = route_extract(dataloader, model_config.obs_len, model_config.pred_len, param_config.random_state, device, param_config.n_clusters)
    ...
    np.save(f'route_{obs_len}_{pred_len}.npy', route)
    ...
    for epoch in range(curr_epoch - 1, epochs):
        ...
        print(f"\r[Epoch {epoch + 1}] [Best Epoch {best_epoch + 1}] [Best ADE {min_true_ade}] [Best FDE {min_true_fde}] {loss_str} ", end='')
