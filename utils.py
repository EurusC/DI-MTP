...
# def plot_predictions_vs_ground_truth(pred, gt, obs, epoch, batch_num, mmsi=None, x_min=None, x_max=None, y_min=None, y_max=None, prob=None):
#     # 将张量转换为 NumPy 数组
#     pred_np = pred.detach().cpu().numpy()   # (batch_size, num_samples, num_timesteps, 2)
#     gt_np = gt.detach().cpu().numpy()       # (batch_size, num_timesteps, 2)
#     gt_np = gt_np.squeeze(1)
...
#             plt.plot(
#                 pred_np[i, j, :, 0], pred_np[i, j, :, 1], color=color, linestyle='--',
#                 marker=pred_markers[i], linewidth=1, markersize=pred_marker_size, label=f'Prediction (MMSI {mmsi[i]})' if j == 0 else ""
...
def num_k_ade_fde_long(param_config, model_config, model, test_func, dataloader, device, pic_name):
    ...
    # 使用Seaborn的lineplot绘制折线图，并区分ADE和FDE
...
#
#     # 获取时间步数
#     num_timesteps = 12  # 预测的时间步数
#
#     # 计算每个预测轨迹与真实轨迹的距离，选择最接近真实轨迹的预测
...
#             ax.scatter(pred_np[i, :t + 1, 0], pred_np[i, :t + 1, 1], marker='o', color='r', s=10,
...
def dataset_vis(dataloader, args):
    ...
    # 创建地图背景，使用 PlateCarree 投影
    fig, ax = plt.subplots(figsize=(6, 6), subplot_kw={'projection': ccrs.PlateCarree()})

    # 设置地图特征（海岸线、土地和海洋），简洁化背景以突出轨迹
